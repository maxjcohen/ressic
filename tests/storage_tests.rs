...
use ressic::{models::Article, storage::{MockStorage, FeedStorage}, Client};
...
fn assert_store_then_get_all<S: FeedStorage>(mut storage: S) {
	...
	storage.store_article(Article { title: title.into(), content: content.into(), id });
	...
	let expected = Article { title: title.into(), content: content.into(), id };
...
fn assert_latest_is_most_recent<S: FeedStorage>(mut storage: S) {
	storage.store_article(Article { title: "first".into(), content: "c1".into(), id: 1 });
	storage.store_article(Article { title: "second".into(), content: "c2".into(), id: 2 });

	let expected = Article { title: "second".into(), content: "c2".into(), id: 2 };
	let latest = storage.get_latest_article();
