...
- [x] **T6 — Fix N+1 reads/writes on POST — add `put_feed` to `FeedStorage`**
...
- [x] **T7 — Remove `set_feed_metadata` from `FeedStorage`** *(resolved by T6)*
  - Prerequisite: T6.
  - Resolution: `set_feed_metadata` is removed as part of T6. No separate action needed.
  - Done when: T6 is complete.

- [x] **T8 — Remove `store_article` struct literal fallbacks** *(resolved by T6)*
  - Prerequisite: T6.
...
- [x] **T15 — Change `Client::store_article` signature from `&mut self` to `&self`** *(superseded by T6)*
